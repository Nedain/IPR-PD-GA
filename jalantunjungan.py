...
translation_awal = translation_field.getSFVec3f()
x_position_awal = translation_awal [0]
translation_akhir = [0.0,0.0,0.0]
x_position_akhir = 0.0
jarak_tempuh = 0.0
...
target_angular_position = position_values[0]  
...
while robot.step(timestep) != -1:
    ...
    #Pembacaan sensor Encoder
    last_position_values[0] = left_encoder.getValue()
    error_enc = target_angular_position - last_position_values[0]
    
    #Pembacaan sensor IMU
    current_pitch_angle = inemu.getRollPitchYaw()[1]        
    error_imu = current_pitch_angle - initial_pitch_angle
    
    #Sistem berjalan 1 meter
    if last_position_values[0] < 30.77 :   
        if error_enc < 2.4: 
            target_angular_position = 4.0 + position_values[0]
        elif error_enc > 2.4:
            target_angular_position = last_position_values[0]
        ...
        if current_time - last_time >= 0.032:
            ...
            time_step += 1
    elif last_position_values[0] >= 30.77:
        target_angular_position = last_position_values[0]
        translation_akhir = translation_field.getSFVec3f()
        x_position_akhir = translation_akhir[0]
        jarak_tempuh = x_position_akhir - x_position_awal
        print(f"Waktu yang dibutuhkan untuk menempuh 1 meter: {current_time}")
    ...
    if current_time-time_awal>15:
        #Menampilkan jarak yang ditempuh oleh robot
        print(f"Posisi Awal: {x_position_awal}")
        print(f"Posisi Akhir: {x_position_akhir}")
        print(f"Jarak yang ditempuh: {jarak_tempuh}")
